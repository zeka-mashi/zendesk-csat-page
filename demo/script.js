...
if (document.URL.indexOf("/index.html") >= 0) {
    if (
        document.URL.indexOf("url_str") < 0 ||
        document.URL.indexOf("zd_id_str") < 0 ||
        document.URL.indexOf("zd_subj_str") < 0
    ) {
    ...
    document.getElementById("survey-ticket-id").innerText = decodeURIComponent(
        getAllUrlParams().id
    );
    try {
        document.getElementById("survey-subject").innerText =
            decodeURIComponent(getAllUrlParams().subj.replace(/\+/g, " "));
    } catch {
        document.getElementById("survey-subject").innerText =
            decodeURIComponent(getAllUrlParams().subj);
    }
    document
        .getElementById("survey-good")
        .addEventListener("click", function () {
            document.location.href =
                decodeURIComponent(getAllUrlParams().url) + "&intention=16";
        });
    document
        .getElementById("survey-bad")
        .addEventListener("click", function () {
            document.location.href =
                decodeURIComponent(getAllUrlParams().url) + "&intention=4";
        });
